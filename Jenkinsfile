...
node('jolin') {
	stage('Environment'){
        ...
        env.ROCKS_VENDOR = "ficus2-android"
        env.ROCKS_LUNCH = "rk3399pro"
    ...
    def environment = docker.build('android-builder:8.x', "--build-arg USER_ID=${uid} --build-arg GROUP_ID=${gid} build-environment")

    environment.inside {
		stage('repo') {
		    repoSync(true, true, "https://github.com/96rocks/manifests.git", "rk3399pro-android-8.1", "rk3399pro-ficus2-vamrs.xml")
		}
	    stage('uboot'){
	    	sh '''#!/bin/bash
	    		#uboot
	    ...
	    stage('kernel'){
	    	sh '''#!/bin/bash
				...
				do
				    ...
				    make ARCH=arm64 rk3399pro-$hardware.img -j16
		...
		stage('make image') {
			sh '''#!/bin/bash
				...
				do
				    ...
				    release_name="${hardware_up}-o-${ROCKS_RELEASE_TIME}_${commitId}"
		...
		stage('release'){
			if (params.RELEASE_TO_GITHUB){
				...
				sh '''#!/bin/bash
					...
					tag=${ROCKS_LUNCH}_Android8.1_${ROCKS_RELEASE_TIME}_${commitId}

	                github-release release \
	                  --target "rk3399pro-android-8.1" \
	                  --tag "${tag}" \
	                  --name "${tag}" \
