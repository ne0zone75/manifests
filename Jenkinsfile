...
node('master') {
    ...
    environment.inside {
		stage('repo') {
		    repoSync(true, true, "https://github.com/96rocks/manifests.git", "rock960-box-7.1", "rock960_box_7.1_release.xml")
		}
	    stage('uboot'){
	    	sh '''#!/bin/bash
	    		#uboot
		...
		stage('make image') {
			sh '''#!/bin/bash
				...
				do
				    ...
				    tar czvf $ROCKS_RELEASE_DIR/${release_name}-rkupdate.tgz rockdev/${release_name}-rkupdate.img
				    tar czvf $ROCKS_RELEASE_DIR/${release_name}-gpt.tgz      rockdev/Image/${release_name}-gpt.img
		...
		stage('release'){
			if (params.RELEASE_TO_GITHUB){
				...
				sh '''#!/bin/bash
					...
					tag=Android_${ROCKS_RELEASE_TIME}_${commitId}

	                github-release release \
	                  --target "rock960-box-7.1" \
	                  --tag "${tag}" \
	                  --name "${tag}" \
					...
					for file in $ROCKS_RELEASE_DIR/*.tgz; do
		                github-release upload \
		                    --tag "${tag}" \
		                    --name "$(basename "$file")" \
		                    --file "$file" &
